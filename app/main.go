...
func galleryHandler(w http.ResponseWriter, r *http.Request) {
	...
	tmpl := template.Must(template.New("gallery").Parse(galleryTemplate))
	tmpl.Execute(w, nil)
}
...
// HTML templates (ported from Node.js templates.js)
...
<body>
    <div class="container">
        ...
        <div class="content">
            {{if .Success}}
            <div class="success" style="text-align: center; direction: rtl; padding: 15px; background: rgba(46, 204, 113, 0.2); border-radius: 12px; margin: 15px 0; border: 1px solid rgba(46, 204, 113, 0.4);">✨ התמונה הועלתה בהצלחה! כעת ניתן לצפות בגלריה.</div>
            {{end}}
            {{if .HasUploaded}}
              <div class="gallery-status">📷 {{if .GalleryExists}}{{len .ImageFiles}} photos{{else}}Gallery ready{{end}} - You have access!</div>
            ...
            {{else}}
              <div class="access-denied" style="text-align: center; padding: 15px; font-size: 16px;">
                {{if .GalleryExists}}
                    📸 מחכות לך {{len .ImageFiles}} תמונות<br>✨ כדי לראות אותן - תעלה תמונה אחת
                {{else}}
            ...
            {{end}}
            {{if .HasUploaded}}
            <a href="/reset" class="reset-link">Reset Session</a>
            {{end}}
        </div>
    </div>
